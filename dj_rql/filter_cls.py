...
class RQLFilterClass:
    ...
    Attributes
    ----------
    * ** MODEL ** (`django.db.models.Model`): The model this filter is for.
    * ** FILTERS ** (`list`): A list or tuple of filters definitions.
    * ** DISTINCT ** (`bool`): If True, a `SELECT DISTINCT` will always be
     executed (default `False`).
    * ** SELECT ** (`bool`): If True, this FilterClass supports the ``select``
     operator (default `False`).
    * ** OPENAPI_SPECIFICATION ** (`RQLFilterClassSpecification`): Class for
     OpenAPI specifications generation (default `RQLFilterClassSpecification`).
    * ** QUERIES_CACHE_BACKEND ** (`Any`): Class for query caching.
    * ** QUERIES_CACHE_SIZE ** (`int`): Default number of cached queries (default 20).
    * ** Q_CLS ** (`Q`): Class for building nodes of the query, generated by
     django (default `Q`).
    *  ** FILTER_TYPES_CLS ** (`FilterTypes`): Class for the mapping of model
     field types to filter types (default `FilterTypes`).
    """
    ...
    def build_q_for_custom_filter(self, data):
        ...
        Args
        ----
        ** data **(`FilterArgs`): Prepared filter data for custom filtering.
        ...
        Returns
        -------
        `django.db.models.Q`: A Django queryset.
        """
    ...
    def build_name_for_custom_ordering(self, filter_name):
        ...
        Args
        ----
        ** filter_name **: Full filter name (f.e. ns1.ns2.filter1)

        Returns
        -------
        `str`: Django field str path
        """
        raise RQLFilterParsingError(details={
    ...
    def optimize_field(self, data: OptimizationArgs):
        ...
        Args
        ----
        ** data ** (`OptimizationArgs`)

        Returns
        -------
        `django.db.models.QuerySet` or `None`: Optimized queryset
        """
        pass
    ...
    def apply_annotations(self, filter_names, queryset=None):
        ...
        ** queryset ** (`django.db.models.QuerySet` or `None`): Queryset for annotation
        """
        if queryset is None:
    ...
    def apply_filters(self, query, request=None, view=None):
        ...
        ** view **: API view
        ...
        Returns
        -------
        `django.db.models.QuerySet` or `None`: Lark AST, Filtered QuerySet
        """
